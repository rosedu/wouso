...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        # Adding model 'IntegerListSetting'
        db.create_table('config_integerlistsetting', (
            ('setting_ptr', self.gf('django.db.models.fields.related.OneToOneField')(to=orm['config.Setting'], unique=True, primary_key=True)),
        ))
        db.send_create_signal('config', ['IntegerListSetting'])


    def backwards(self, orm):
        # Deleting model 'IntegerListSetting'
        db.delete_table('config_integerlistsetting')


    models = {
        'config.integerlistsetting': {
            'Meta': {'object_name': 'IntegerListSetting', '_ormbases': ['config.Setting']},
