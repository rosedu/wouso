...
def player(user):
    ...
    rel_data="%s %s %s %s %s 1" % (user, user.points, player_avatar(user), user.level_no, user.id)
...
def player_simple(user):
    ...
    link = reverse('wouso.interface.profile.views.user_profile', args=(user.id,))
    rel_data_simple ="%s %s %s %s %s 1" % (user, user.points, player_avatar(user), user.level_no, user.id)
