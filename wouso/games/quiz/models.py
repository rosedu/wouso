...
from wouso.core.qpool import \
    register_category, get_questions_with_tag_and_category
from wouso.core.qpool.models import Question, Tag


class QuizCategory(models.Model):
    name = models.CharField(max_length=100, unique=True)
    logo = models.ImageField(upload_to=settings.MEDIA_ARTIFACTS_DIR,
...
class Quiz(models.Model):
    ...
    def calculate_reward(self, responses):
        ...
        for question_id, checked_answer_id in responses.iteritems():
            ...
            correct_answer_id = [ans.id for ans in question.answers
                                 if ans.correct]
...
class UserToQuiz(models.Model):
    ...
    def _give_bonus(self, points, gold):
        if self.best_attempt is not None:
            if points > self.best_attempt.points:
                ...
                add_activity(
                    self.user,
                    _('received {points} points and {gold} gold bonus '
    ...
    def can_play_again(self):
        if self.all_attempts.count():
            return (datetime.now() -
                    self.last_attempt.date).days >= self.quiz.another_chance
        return True
    ...
    def days_until_can_replay(self):
        if self.can_play_again():
            return 0
        return self.quiz.another_chance - (datetime.now() -
                                           self.last_attempt.date).days
