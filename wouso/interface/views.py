...
def ajax_get(request, model, id=0):
    ...
    return HttpResponsemessage(serializers.serialize('json', (obj,)))
...
def report(request,id):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            signals.addActivity.send(sender=None, user_from=user_from.get_profile().get_extension(Player), user_to=user_to.get_profile().get_extension(Player), action="report", game=None)
