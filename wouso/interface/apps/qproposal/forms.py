...
class ProposedQuestionForm(forms.Form):

    text = forms.CharField(max_length=500, widget=forms.Textarea)
    category = forms.ChoiceField(choices=[(cat.name, cat.name) for cat in Category.objects.all()], widget=forms.Select(attrs={
        'id': 'select-category'}))

    def __init__(self, nr_ans=6, data=None):
        super(ProposedQuestionForm, self).__init__(data)
        self.nr_ans = nr_ans
        for i in range(nr_ans):
            self.fields['answer_%d' % i] = forms.CharField(max_length=100,
                                    widget=forms.Textarea, required=False)
