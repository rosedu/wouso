...
class CpanelViewsTest(WousoTest):
    ...
    def test_karma_view(self):
        ...
        response = self.client.get(reverse('karma'), kwargs={'races': race, 'groups': group})
    ...
    def test_karma_group_view(self):
        race = Race.objects.create(name='Race_test_1', can_play=True)
        group = PlayerGroup.objects.create(name='Group_test_1', parent=race)
        player = User.objects.create(username='Sibi', password='test').get_profile()
        group.players.add(player)

        response = self.client.get(reverse('karma_group', kwargs={'group': group.id}))

        self.assertContains(response, 'Sibi')
        self.assertContains(response, 'Group_test_1')
